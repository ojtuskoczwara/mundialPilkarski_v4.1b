package interfejsUzytkownikaModul.mvc.model.entity;

public class Rozgrywka {
    private int idRozgrywki;
    private int idGrupy;
    private int idGrupy1;
    private int idReprezentacji1;
    private int idGrupy2;
    private int idReprezentacji2;
    private int idMundialu;
    private int idTypuGrupy;

    public int getIdRozgrywki() {
        return idRozgrywki;
    }

    public void setIdRozgrywki(int idRozgrywki) {
        this.idRozgrywki = idRozgrywki;
    }

    public int getIdGrupy1() {
        return idGrupy1;
    }

    public void setIdGrupy1(int idGrupy1) {
        this.idGrupy1 = idGrupy1;
    }

    public int getIdGrupy() {
        return idGrupy;
    }

    public void setIdGrupy(int idGrupy) {
        this.idGrupy = idGrupy;
    }

    public int getIdReprezentacji1() {
        return idReprezentacji1;
    }

    public void setIdReprezentacji1(int idReprezentacji1) {
        this.idReprezentacji1 = idReprezentacji1;
    }

    public int getIdGrupy2() {
        return idGrupy2;
    }

    public void setIdGrupy2(int idGrupy2) {
        this.idGrupy2 = idGrupy2;
    }

    public int getIdReprezentacji2() {
        return idReprezentacji2;
    }

    public void setIdReprezentacji2(int idReprezentacji2) {
        this.idReprezentacji2 = idReprezentacji2;
    }

    public int getIdMundialu() {
        return idMundialu;
    }

    public void setIdMundialu(int idMundialu) {
        this.idMundialu = idMundialu;
    }

    public int getIdTypuGrupy() {
        return idTypuGrupy;
    }

    public void setIdTypuGrupy(int idTypuGrupy) {
        this.idTypuGrupy = idTypuGrupy;
    }
}
